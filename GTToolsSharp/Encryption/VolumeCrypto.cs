...
public class VolumeCrypto
{
	...
	public static void DecryptBuffer(Span<byte> input, Span<byte> output, int size, Span<byte> decryptTable, ulong offset)
	{
		...
		if (inPos % 8 == 0 && outPos % 8 == 0)
		{
			...
			else
			{
				...
				if (inPos % 8 != 0)
				{
					...
					if (countToAligned > 0)
					{
						var bits0_ = decryptTable[bO1..];
						var bits1_ = decryptTable[(bO2 + 0x88)..];
						var bits2_ = decryptTable[(bO3 + 0x120)..];
						var bits3_ = decryptTable[(bO4 + 0x1D8)..];
		...
		else
		{
			...
			var bits0 = decryptTable[bO1..];
			var bits1 = decryptTable[bO2..];
			var bits2 = decryptTable[bO3..];
			var bits3 = decryptTable[bO4..];
	...
	static readonly byte[] unkBitTable =
    [
