...
class BsDiff
{
				...
				Span<byte> newB, int newsize, int st, int en, out int pos)
	{
		...
		if (en - st < 2) {
			x = MatchLength(old[I[st]..], oldsize - I[st], newB, newsize);
			y = MatchLength(old[I[en]..], oldsize - I[en], newB, newsize);
		...
		x = st + (en - st) / 2;
		if (old[I[x]..].MemoryCompare(newB, Math.Min(oldsize - I[x], newsize) ) < 0)
