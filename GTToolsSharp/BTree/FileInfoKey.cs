...
public class FileInfoKey : IBTreeKey<FileInfoKey>
{
    ...
    public void Serialize(ref BitStream bs)
    {
        ...
        bs.WriteVarInt(FileIndex);
        bs.WriteVarInt(CompressedSize);
        if (Flags.HasFlag(FileInfoFlags.Compressed) || (UsesMultipleVolumes && Flags.HasFlag(FileInfoFlags.PDIZIPCompressed)))
            bs.WriteVarInt(UncompressedSize);

        if (UsesMultipleVolumes)
            bs.WriteVarInt(VolumeIndex);

        bs.WriteVarInt(SectorOffset);
    }

    public uint GetSerializedKeySize()
    {
        uint keyLength = 1;
        keyLength += (uint)BitStream.GetSizeOfVarInt(FileIndex);
        keyLength += (uint)BitStream.GetSizeOfVarInt(CompressedSize);
        if (Flags.HasFlag(FileInfoFlags.Compressed) || (UsesMultipleVolumes && Flags.HasFlag(FileInfoFlags.PDIZIPCompressed)))
            keyLength += (uint)BitStream.GetSizeOfVarInt(UncompressedSize);

        if (UsesMultipleVolumes)
            keyLength += (uint)BitStream.GetSizeOfVarInt(VolumeIndex);

        keyLength += (uint)BitStream.GetSizeOfVarInt(SectorOffset);
